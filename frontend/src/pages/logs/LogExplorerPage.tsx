...
export function LogExplorerPage() {
  ...
  const events = logs.data?.pages.flatMap((page) => page.events) ?? [];
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-950">Log Explorer</h1>
        <p className="text-sm text-slate-500">
          Search normalized events in your project.
        </p>
      </div>
      <form
        onSubmit={submit}
      ...
      >
        ...
        <input
          ...
          className="rounded-lg border border-slate-300 px-3 py-2"
        />
        <button className="rounded-lg bg-sky-600 px-4 py-2 font-semibold text-white hover:bg-sky-700">
          Search
        </button>
      </form>
      {logs.isLoading && (
      ...
      {logs.isError && (
        <p role="alert" className="rounded-xl bg-rose-50 p-6 text-rose-700">
          {logs.error.message}
        </p>
      )}
      {logs.data && (
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                {events.map((event) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded px-2 py-1 text-xs font-bold ${levelStyles[event.level] ?? levelStyles.DEBUG}`}
                      >
                        {event.level}
                      </span>
                    </td>
                    ...
                    <td className="max-w-xl truncate px-4 py-3 text-slate-700">
                      {event.message}
                    </td>
                  </tr>
                ))}
          ...
          {logs.hasNextPage && (
            <div className="border-t border-slate-100 p-4 text-center">
              ...
              >
                {logs.isFetchingNextPage ? "Loading more…" : "Load more"}
              </button>
            </div>
          )}
        </div>
      ...
      {selectedId && (
        <aside
          ...
          className="fixed inset-y-0 right-0 z-50 w-full max-w-xl overflow-auto border-l border-slate-200 bg-white p-6 shadow-2xl"
        >
          <button
