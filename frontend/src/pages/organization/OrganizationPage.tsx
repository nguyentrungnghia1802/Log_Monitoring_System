...
export function OrganizationPage() {
  ...
  const organizationMutation = useMutation({
    mutationFn: () =>
      updateCurrentOrganization(name, organizationQuery.data?.settings ?? {}),
    onSuccess: (updated) => {
  ...
  const memberMutation = useMutation({
    mutationFn: ({
      ...
      userId: string;
      changes: { active?: boolean; role?: OrganizationRole };
    ...
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Manage organization settings and access without editing MongoDB
          directly.
        </p>
      </div>
      ...
      <section className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        >
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              The password is sent once to create the account and is never
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">
              {organizationQuery.data.memberCount} account(s) in this
              organization.
            </p>
          </div>
        </div>
                ...
                {members.map((member) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <span
                        className={
                          member.active ? "text-emerald-700" : "text-slate-500"
                        }
                      >
                    ...
                    <td className="px-3 py-3 text-right">
                      <button
                        onClick={() => {
                          if (
                            !member.active ||
                            window.confirm(`Disable ${member.username}?`)
                          ) {
                            memberMutation.mutate({
                              userId: member.id,
                      ...
                      <button
                        onClick={() => {
                          if (
                            window.confirm(
                              `Remove ${member.username} from this organization?`,
                            )
                          )
        ...
        {(memberMutation.error || removeMutation.error) && (
          <p className="mt-4 text-sm text-rose-700">
            {(memberMutation.error ?? removeMutation.error)?.message}
          </p>
        )}
      </section>
    </main>
