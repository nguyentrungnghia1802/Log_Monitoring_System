    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Triggered occurrences and notification delivery status
        </p>
      </div>
      ...
      {projectsQuery.isLoading || isLoading ? (
        ...
        <div className="rounded-xl border border-red-800 bg-red-950/40 p-5 text-red-200">
          ...
          <button
            ...
            className="mt-3 rounded bg-red-800 px-3 py-1 text-sm"
          >
            Retry
          </button>
        </div>
      ...
      {selectedAlertId && (
        <section
          aria-label="Alert occurrence detail"
        ...
        >
          ...
          {detailQuery.isLoading ? (
            <p className="mt-4">Loading detail...</p>
          ) : detailQuery.error ? (
            <p className="mt-4 text-red-300">{detailQuery.error.message}</p>
          ) : (
            detailQuery.data && (
              <div className="mt-5 space-y-5">
                <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    ...
                    <dd>{detailQuery.data.ruleName}</dd>
                  </div>
                  <div>
                    <dt className="text-slate-500">Why triggered</dt>
                    <dd>
                      {detailQuery.data.observedValue} events reached threshold{" "}
                      {detailQuery.data.threshold}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-slate-500">Window</dt>
                    <dd>
                      {new Date(detailQuery.data.windowStart).toLocaleString()}{" "}
                      – {new Date(detailQuery.data.windowEnd).toLocaleString()}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-slate-500">Acknowledgement</dt>
                    <dd>
                      {detailQuery.data.acknowledgedAt
                        ? `${detailQuery.data.acknowledgedBy} at ${new Date(detailQuery.data.acknowledgedAt).toLocaleString()}`
                        : "Not acknowledged"}
                    </dd>
                  </div>
                </dl>
                <div>
                  ...
                  {(detailQuery.data.deliveryAttempts ?? []).length === 0 ? (
                    ...
                    <ol className="mt-2 space-y-2">
                      {detailQuery.data.deliveryAttempts.map((attempt) => (
                        ...
                        >
                          ...
                          : <strong>{attempt.status}</strong> at{" "}
