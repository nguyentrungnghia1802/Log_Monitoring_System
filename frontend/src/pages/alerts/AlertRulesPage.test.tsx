...
describe("AlertRulesPage", () => {
  it("creates a validated project-scoped rule with level and event-type filters", async () => {
    ...
    vi.mocked(createAlertRule).mockResolvedValue({
      id: "rule-1",
      projectId: "project-1",
      name: "Queue spike",
      enabled: true,
      levels: ["ERROR"],
      eventTypes: ["QUEUE_FAILED"],
      windowSeconds: 60,
      threshold: 10,
      cooldownSeconds: 300,
    });
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "+ Create Alert Rule" }),
    );
    fireEvent.change(
      screen.getByPlaceholderText("e.g. Queue Service Error Spike"),
      { target: { value: "Queue spike" } },
    );
    fireEvent.change(screen.getByLabelText("Event types"), {
      target: { value: "QUEUE_FAILED" },
    });
