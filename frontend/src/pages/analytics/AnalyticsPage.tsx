...
import type { AnalyticsSummaryResponse, AnalyticsHistogramResponse } from '../../types/analytics'
import { fetchAnalyticsSummary, fetchAnalyticsHistogram } from '../../services/analyticsApi'
...
export function AnalyticsPage() {
    ...
    const [histogram, setHistogram] = useState<AnalyticsHistogramResponse | null>(null)
    ...
    const loadAnalytics = async () => {
        ...
        try {
            ...
            const interval = timeRange === '1h' ? '5m' : timeRange === '24h' ? '1h' : '1d'

            const [sumRes, histRes] = await Promise.all([
                fetchAnalyticsSummary({ projectId, startTime, endTime, environment: environment || undefined, service: service || undefined }),
                fetchAnalyticsHistogram({ projectId, startTime, endTime, interval, environment: environment || undefined, service: service || undefined }),
            ])
    ...
    return (
        <div className="mx-auto max-w-7xl px-6 py-8 space-y-8">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500">
                        Log volume trends, error rate distributions, and top recurring exceptions.
                    </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {['1h', '24h', '7d'].map((range) => (
                        <button
                            ...
                            className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-all ${timeRange === range
            ...
            {loading && (
                <div className="text-xs text-sky-600 font-semibold animate-pulse">Updating analytics...</div>
            )}
            ...
            <div className="grid gap-5 md:grid-cols-4">
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-1">
                    <span className="text-xs font-semibold text-slate-400">Total Ingested Logs</span>
                    <div className="text-3xl font-extrabold text-slate-900">
                        {summary ? summary.totalLogs.toLocaleString() : '-'}
                    </div>
                    <span className="text-[11px] text-slate-500">In selected time range</span>
                </div>

                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-1">
                    <span className="text-xs font-semibold text-slate-400">Error Rate</span>
                    <div className={`text-3xl font-extrabold ${summary && summary.errorRatePercentage > 5 ? 'text-rose-600' : 'text-emerald-600'}`}>
                        {summary ? `${summary.errorRatePercentage}%` : '-'}
                    </div>
                    <span className="text-[11px] text-slate-500">Errors & Warnings ratio</span>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-1">
                    <span className="text-xs font-semibold text-slate-400">Top Error Fingerprint</span>
                    <div className="text-sm font-bold text-rose-700 truncate">
                        {summary && summary.topErrors.length > 0 ? summary.topErrors[0].errorFingerprint : 'None'}
                    </div>
                    <span className="text-[11px] text-slate-500">
                        {summary && summary.topErrors.length > 0 ? `${summary.topErrors[0].count} occurrences` : 'Clean'}
                    </span>
                </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs space-y-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-base font-bold text-slate-900">Log Volume Time-Series</h2>
                    <span className="text-xs font-medium text-slate-400">Interval: {histogram?.interval || '1h'}</span>
                </div>
                ...
                {histogram && histogram.buckets.length > 0 ? (
                    <div className="h-44 flex items-end gap-1.5 pt-6 pb-2 border-b border-slate-100">
                        {histogram.buckets.map((bucket, idx) => {
                            const heightPct = Math.max((bucket.total / maxBucketTotal) * 100, 4)
                            ...
                            return (
                                <div key={idx} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                                    ...
                                    <div className="absolute bottom-full mb-2 hidden group-hover:block bg-slate-900 text-white text-[10px] p-2 rounded-lg shadow-lg z-20 whitespace-nowrap">
                                        ...
                                        <div className="text-rose-300 font-bold">Errors: {bucket.errorCount}</div>
                                        <div className="text-amber-300">Warns: {bucket.warnCount}</div>
                                    ...
                                    >
                                        {bucket.errorCount > 0 && (
                                            <div style={{ height: `${errorPct}%` }} className="bg-rose-500 w-full" />
                                        )}
                                        {bucket.warnCount > 0 && (
                                            <div style={{ height: `${warnPct}%` }} className="bg-amber-400 w-full" />
                                        )}
                                        {bucket.infoCount > 0 && (
                                            <div style={{ height: `${infoPct}%` }} className="bg-sky-500 w-full" />
                                        )}
                                    </div>
                                </div>
                            )
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs space-y-4">
                    <h2 className="text-base font-bold text-slate-900">Top Error Fingerprints</h2>
                    <div className="space-y-3">
                        {summary?.topErrors.length === 0 && (
                            <div className="text-xs text-slate-400 py-4 text-center">No errors recorded.</div>
                        )}
                        {summary?.topErrors.map((err, idx) => (
                            <div key={idx} className="p-3 bg-rose-50/50 rounded-xl border border-rose-100 space-y-1">
                                <div className="flex items-center justify-between">
                                    <span className="text-xs font-bold text-rose-800 font-mono">{err.errorFingerprint}</span>
                                    <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-rose-100 text-rose-700">
                                        {err.count}
                                    </span>
                                </div>
                                {err.sampleMessage && (
                                    <p className="text-xs text-slate-600 truncate">{err.sampleMessage}</p>
                                )}
                            </div>
                        ))}
                    </div>
                ...
                {/* Top Services Volume */}
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs space-y-4">
                    <h2 className="text-base font-bold text-slate-900">Top Services Volume</h2>
                    <div className="space-y-3">
                        {summary?.topServices.length === 0 && (
                            <div className="text-xs text-slate-400 py-4 text-center">No service data available.</div>
                        )}
                        {summary?.topServices.map((srv, idx) => {
                            const pct = summary.totalLogs > 0 ? Math.round((srv.count / summary.totalLogs) * 100) : 0
                            return (
                                <div key={idx} className="space-y-1">
                                    <div className="flex justify-between text-xs font-medium">
                                        <span className="text-slate-800 font-bold">{srv.service}</span>
                                        <span className="text-slate-500">{srv.count.toLocaleString()} ({pct}%)</span>
                                    </div>
