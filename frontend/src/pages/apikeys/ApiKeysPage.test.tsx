...
describe("ApiKeysPage", () => {
  ...
  it("renders metadata without rendering or persisting the raw secret", async () => {
    ...
    expect(
      screen.queryByText(secretResponse.rawApiKey),
  ...
  it("shows a one-time secret with copy confirmation and clears it when dismissed", async () => {
    ...
    expect(await screen.findByTestId("one-time-api-key")).toHaveTextContent(
    ...
    await waitFor(() =>
      expect(screen.queryByTestId("one-time-api-key")).not.toBeInTheDocument(),
    );
  });
