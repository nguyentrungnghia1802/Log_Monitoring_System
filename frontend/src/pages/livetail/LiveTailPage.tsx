...
export function LiveTailPage() {
    ...
    useEffect(() => {
        if (!paused) {
            if (bufferedEventsRef.current.length > 0) {
                setEvents((prev) => [...prev, ...bufferedEventsRef.current].slice(-200))
    ...
    const filteredEvents = events.filter((e) => {
        if (levelFilter && e.level.toUpperCase() !== levelFilter.toUpperCase()) return false
        if (serviceFilter && !e.service.toLowerCase().includes(serviceFilter.toLowerCase())) return false
        if (keywordFilter && !e.message.toLowerCase().includes(keywordFilter.toLowerCase())) return false
        return true
    })
        ...
        <div className="mx-auto max-w-7xl px-6 py-8 space-y-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                    <h1 className="text-2xl font-bold tracking-tight text-slate-900">Live Tail Stream</h1>
                    <span
                    ...
                    >
                        <span className={`h-2 w-2 rounded-full ${connected ? 'bg-emerald-500 animate-ping' : 'bg-amber-500'}`} />
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`rounded-xl px-4 py-2 text-xs font-bold transition-all shadow-xs ${paused
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    {['', 'ERROR', 'WARN', 'INFO', 'DEBUG'].map((lvl) => (
                        <button
                            ...
                            className={`rounded-md px-2.5 py-1 font-semibold border transition-all ${levelFilter === lvl
            ...
            <div className="rounded-2xl border border-slate-900 bg-slate-950 p-4 font-mono text-xs shadow-xl overflow-hidden min-h-[500px] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto pt-3 space-y-1.5 max-h-[600px]">
                    ...
                    {filteredEvents.map((evt, idx) => (
                        <div key={idx} className="flex items-start gap-3 hover:bg-slate-900/60 p-1.5 rounded transition-colors">
                            <span className="text-slate-500 whitespace-nowrap">
                                {new Date(evt.timestamp).toLocaleTimeString()}
                            </span>
                            <span className={`px-1.5 py-0.5 rounded text-[10px] whitespace-nowrap ${getLevelBadge(evt.level)}`}>
                                {evt.level}
                            </span>
