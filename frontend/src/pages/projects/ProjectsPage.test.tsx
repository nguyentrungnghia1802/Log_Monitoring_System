...
describe("ProjectsPage", () => {
  ...
  it("edits default and per-level retention with explicit TTL semantics", async () => {
    vi.mocked(fetchProjects).mockResolvedValue([
      {
        ...
        retention: { defaultDays: 7, levelOverrides: { ERROR: 30 } },
        settings: {},
        services: ["checkout"],
        recentIngestion: {
    ...
    expect(retentionForm).toHaveTextContent(
      /existing expireAt values are not backfilled/i,
    );
    await waitFor(() =>
      expect(screen.getByLabelText("ERROR retention days")).toHaveValue(30),
    );
    fireEvent.change(screen.getAllByLabelText("Default retention (days)")[1], {
      target: { value: "14" },
    });
    fireEvent.change(screen.getByLabelText("WARN retention days"), {
      target: { value: "21" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Save retention" }));

    await waitFor(() =>
      expect(updateProjectRetention).toHaveBeenCalledWith("project-1", {
        defaultDays: 14,
