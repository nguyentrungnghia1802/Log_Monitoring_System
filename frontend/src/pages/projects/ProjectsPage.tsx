...
export function ProjectsPage() {
  ...
  useEffect(() => {
    if (selectedProject) {
      setEditForm({
        ...
        environments: selectedProject.environments.join(", "),
      });
      setRetentionDays(String(selectedProject.retention.defaultDays));
      setRetentionOverrides(
        Object.fromEntries(
          RETENTION_LEVELS.map((level) => [
            level,
            selectedProject.retention.levelOverrides[level]?.toString() ?? "",
          ]),
        ),
      );
    }
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      createProject({
        ...
        retention: {
          defaultDays: Number(createForm.defaultDays),
          levelOverrides: {},
        },
      }),
    onSuccess: (created) => {
  ...
  const retentionMutation = useMutation({
    mutationFn: () =>
      updateProjectRetention(selectedProject!.id, {
        defaultDays: Number(retentionDays),
        levelOverrides: Object.fromEntries(
          RETENTION_LEVELS.filter(
            (level) => retentionOverrides[level] !== "",
  ...
  const submitCreate = (event: FormEvent) => {
    ...
    if (
      ...
      environmentList(createForm.environments).length > 0
    ) {
  ...
  const submitUpdate = (event: FormEvent) => {
    ...
    if (
      selectedProject &&
      editForm.name.trim() &&
      environmentList(editForm.environments).length > 0
    ) {
  ...
  const retentionValid =
    ...
    RETENTION_LEVELS.every((level) =>
      validRetentionDays(retentionOverrides[level] ?? "", true),
    );
  const storageWindowRatio = validRetentionDays(retentionDays)
    ...
    <main className="mx-auto max-w-7xl space-y-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Onboard monitored applications, configure environments and inspect
          recent activity.
        </p>
      </div>
        ...
        >
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              The project key is a stable lowercase slug and cannot be changed
              after creation.
            </p>
          </div>
          ...
          <label className="block text-sm font-medium text-slate-700">
            ...
            <input
              ...
              placeholder="LINE Smart Queue Assistant"
              className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
            />
          </label>
          <label className="block text-sm font-medium text-slate-700">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {projects.length} project(s) visible in your organization scope.
              </p>
            </div>
          </div>
          {projects.length === 0 ? (
            <p className="mt-5 rounded-lg bg-slate-50 p-5 text-sm text-slate-600">
              No projects found. Create the first monitored application above.
            </p>
          ) : (
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {projects.map((project) => (
                <button
                  ...
                  className={`rounded-xl border p-4 text-left transition ${selectedProjectId === project.id ? "border-sky-400 bg-sky-50" : "border-slate-200 hover:border-sky-300"}`}
                >
                  ...
                  <p className="mt-3 text-xs text-slate-600">
                    ...
                    {project.recentIngestion.eventsLast24Hours.toLocaleString()}{" "}
                    events / 24h
                  </p>
                </button>
              ))}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {selectedProject.active && (
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      `Deactivate ${selectedProject.name}? Ingestion will be rejected.`,
                    )
                  )
          ...
          <div className="mt-5 grid gap-6 lg:grid-cols-2">
            ...
            >
              ...
              <div className="rounded-lg border border-sky-200 bg-sky-50 p-3 text-xs text-sky-900">
                MongoDB TTL cleanup is asynchronous, so expiry is not exact to
                the second. Changes apply only to future ingested events;
                existing <code>expireAt</code> values are not backfilled.
              </div>
              <label className="block text-sm font-medium text-slate-700">
                Default retention (days)
                <input
                ...
                <div className="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {RETENTION_LEVELS.map((level) => (
                    ...
                    >
                      {level}
                      <input
                        aria-label={`${level} retention days`}
                        min={1}
                        max={3650}
                        type="number"
                        value={retentionOverrides[level] ?? ""}
                        onChange={(event) =>
                          setRetentionOverrides({
                            ...retentionOverrides,
              ...
              {storageWindowRatio !== null && (
                <p className="rounded-lg bg-slate-50 p-3 text-xs text-slate-600">
                  At a steady event rate, the default policy retains roughly{" "}
                  <strong>{storageWindowRatio.toFixed(1)}×</strong> the
                  event-days of a 7-day policy. This is a planning ratio, not a
                  byte estimate.
                </p>
              )}
              {!retentionValid && (
              ...
              <button
                disabled={retentionMutation.isPending || !retentionValid}
                className="rounded-lg bg-sky-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
              >
          ...
          <div className="mt-6 grid gap-3 text-sm sm:grid-cols-3">
            <div className="rounded-lg bg-slate-50 p-3">
              <p className="text-xs text-slate-500">Services discovered</p>
            ...
            <div className="rounded-lg bg-slate-50 p-3">
              <p className="text-xs text-slate-500">Events / last 24h</p>
              <p className="mt-1 font-semibold text-slate-900">
                {selectedProject.recentIngestion.eventsLast24Hours.toLocaleString()}{" "}
                (
                {selectedProject.recentIngestion.errorEventsLast24Hours.toLocaleString()}{" "}
                errors)
              </p>
            </div>
            <div className="rounded-lg bg-slate-50 p-3">
              ...
              <p className="mt-1 font-semibold text-slate-900">
                {formatTimestamp(
                  selectedProject.recentIngestion.lastReceivedAt,
                )}
              </p>
            </div>
          </div>
