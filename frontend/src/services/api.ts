...
export async function fetchLogs(
  ...
  if (params.errorFingerprint)
    query.set("errorFingerprint", params.errorFingerprint);
  if (params.search) query.set("search", params.search);
  if (params.cursor) query.set("cursor", params.cursor);
  if (params.limit) query.set("limit", params.limit.toString());

  return apiRequest<LogSearchResponse>(
    `/projects/${params.projectId}/logs?${query.toString()}`,
  );
}
...
export async function toggleAlertRule(
  ...
  return apiRequest<AlertRule>(
    `/projects/${projectId}/alert-rules/${ruleId}/${action}`,
    { method: "POST" },
  );
}

export async function deleteAlertRule(projectId: string, ruleId: string) {
  return apiRequest<void>(`/projects/${projectId}/alert-rules/${ruleId}`, {
    method: "DELETE",
  });
}
...
export async function fetchAlertById(
  ...
  return apiRequest<AlertOccurrence>(
...
export async function retryAlertNotification(
  ...
  return apiRequest<AlertOccurrence>(
    `/projects/${projectId}/alerts/${alertId}/retry-notification`,
    { method: "POST" },
  );
}
